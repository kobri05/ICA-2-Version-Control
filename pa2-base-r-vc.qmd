...
```{r}
#| label: One Long Vector
my_symbols <- c(lower_case, upper_case, punctuation)
# Added 'c' before parentheses to make command work.
```

3.  Turn the `my_symbols` vector into a data frame, with one column named "symbol".

```{r}
#| label: Symbol Data Frame
my_symbols <- data.frame(symbol = my_symbols)
# a dot was added to the dataframe command
```

4.  Find the total number of symbols we have in our data frame.

```{r}
#| label: Total Number of Symbols in DF
len <- length(my_Symbols$symbols)
# Added variable $symbols to command len.
```
...
#Changed percent sign between my_symbols and num to a '$'
```
